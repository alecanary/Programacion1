...
public class Ejercicios1 {
	...
	private void jugarAdivinaNumero(int numeroAdivinar) {
		...
		do { // bucle del juego
			...
			while (error1) {
				try {
					System.out.println("Teclee un número entero, para salir pulse (Q/q) y Enter");
			...
			for (int i = 0; i < intentos.length; i++) {
				try {
					System.out.printf("%d. \t%d\t%s\n", i + 1, intentos[i].getNumeroIntento(),
			...
			// tenemos un numero tecleado válido
	...
	private int[] generarIntervalo() {
		...
		while (error1) {
			...
			try {
				...
				if (min >= max) {
					System.out.println("Min debe ser menor que max.");
					error1 = true;
					continue;
		...
		System.out.println("VALIDACION SUPERADA");
